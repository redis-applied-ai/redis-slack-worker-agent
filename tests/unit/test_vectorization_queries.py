    ...
    async def test_list_content_files_in_s3_folder_blog(self, mock_boto_client):
        ...
        mock_response = {
            "Contents": [
                ...
                {"Key": "processed/blog_text/2025-09-10/other.txt"},  # Should be filtered out
                {"Key": "processed/blog_text/2025-09-10/subfolder/"},  # Directory, filtered out
            ]
        }
        mock_s3_client.list_objects_v2.return_value = mock_response
        
        # Test the function
        result = await list_content_files_in_s3_folder("test-bucket", "blog", "2025-09-10")
    ...
    async def test_list_content_files_in_s3_folder_repo(self, mock_boto_client):
        ...
        mock_response = {
            "Contents": [
                ...
                {"Key": "processed/repo/2025-09-10/readme.md"},  # Should be filtered out for repo
            ]
        }
        mock_s3_client.list_objects_v2.return_value = mock_response
        
        # Test the function
        result = await list_content_files_in_s3_folder("test-bucket", "repo", "2025-09-10")
